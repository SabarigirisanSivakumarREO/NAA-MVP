---
name: Demo & Stakeholder Briefing Guide
status: validated
version: v2.3
description: Internal demo script and stakeholder Q&A for REO Digital briefings. Aligned with master plan v2.3 + PRD v1.2.
date: 2026-04-15 (content aligned to v2.3 on 2026-04-24)
---
...
**Agent 2: The Analysis Agent** — This is the CRO brain. Once the browser agent has loaded a page, the analysis agent scans everything: every button, every form, every image, every heading. Then it evaluates what it sees against 30 expert rules in the MVP pilot (scaling to 100 at full corpus) — things like 'does this checkout form have too many fields?' or 'is the primary CTA visible without scrolling?' Every finding goes through three filters before anyone sees it.
...
A: In MVP, the browser agent uses standard Playwright with overlay dismissal, realistic delays, and a rate limiter (2s per domain, 10/min for unknowns). Stealth technology — mouse-movement mimicry, typing patterns, fingerprint rotation — is on the v1.1 roadmap for tougher sites. If a site detects us (Amazon is aggressive about this), the system pauses and asks a human to help. We never force through a block.

**Q: "What are the 100 rules based on?"**
A: Three research-backed sources: Baymard Institute (e-commerce UX), Jakob Nielsen's usability heuristics, and Robert Cialdini's persuasion principles. Full corpus targets ~100 rules (50 Baymard + 35 Nielsen + 15 Cialdini); MVP ships 30 (~15 Baymard + ~10 Nielsen + ~5 Cialdini) with the remaining 70 authored in parallel post-MVP. Every rule has an industry benchmark attached — for example, "checkout forms should have 6-8 fields (Baymard research)" or "primary CTA must be above the fold (Nielsen Norman Group)."
...
### What Gets Built
- Browser launcher (MVP: default Playwright + overlay dismissal; stealth package deferred to v1.1)
...
### Say This (Non-Technical)
"This is where we load the expert knowledge. MVP ships with 30 rules; the full corpus is 100 — the rules our CRO consultants know by heart. Things like: 'A checkout form should have 6-8 fields maximum' (Baymard Institute research), 'The primary call-to-action should be visible without scrolling' (Nielsen Norman Group), 'Social proof should be near the purchase decision point' (Cialdini's persuasion research).
...
This knowledge is our intellectual property. In MVP it lives in a private git repository (heuristics-repo/); AES-256-GCM encryption at rest ships before the first external pilot (v1.1)."

### Say This (Technical Team)
"30 heuristics in JSON for MVP pilot (100 at full corpus); Zod-validated schema. Required benchmark field: quantitative (value, source, unit, comparison, threshold_warning, threshold_critical) or qualitative (standard, source, positive_exemplar, negative_exemplar). Two-stage filtering: filterByBusinessType in audit_setup (30→~20 for ecommerce; 100→~60-70 at full corpus), filterByPageType in page_router (→15-20). Plain JSON in private repo for MVP; AES-256-GCM encryption at rest is v1.1 before first external pilot. GR-012 validates benchmark claims. Three reliability tiers determining auto-publish eligibility."

### What Gets Built
- 30 heuristic rules authored for MVP pilot with benchmarks (~15 Baymard + ~10 Nielsen + ~5 Cialdini); full corpus of 100 targeted post-MVP

...
delta:
  ...
  changed:
    ...
    - v0.4 → v0.5 — companion update to spec.md v0.7 Tiered Verification Methodology + plan.md v0.7 + tasks.md v0.7 + new `neural-heuristic-reviewer` skill + `HeuristicSchemaExtended.ai_review` optional schema field amendment (2026-05-09). **Contract-shape change (additive, non-breaking):** new optional `ai_review` field on HeuristicSchemaExtended (`packages/agent-core/src/analysis/heuristics/types.ts`). Phase 6 loader unchanged — old content without `ai_review` still parses; new content with `ai_review` parses identically; the field is opaque to the loader (only `neural-heuristic-reviewer` skill writes it; only the human-gate UI reads it). **Producer added (Stage 2b):** `neural-heuristic-reviewer` skill writes `ai_review` block to `.heuristic-drafts/<id>.review.json` then commits land it as part of the heuristic JSON in `heuristics-repo/<pack>/<id>.json`. **Consumer mapping (R20 audit):** ai_review block consumed only by (a) the human gate UI at Stage 2c (renders for fast-stamp); (b) optional future analytics on FLAG_FOR_HUMAN rates per pack (post-MVP); NOT consumed by Phase 6 loader, NOT consumed by Phase 4b filter, NOT consumed by Phase 7 EvaluateNode (R6 boundary preserved — ai_review.* never serialized to LLM evaluate prompt). **Cost impact:** drafting cost ~$5-10 (unchanged) + new AI-review skill cost ~$1.80 (30 heuristics × ~$0.06 with WebFetch enabled) = total ~$7-12 phase LLM spend, well within $25 ceiling. **Storage impact:** ai_review field is small (~1KB per heuristic × 30 = ~30KB; trivial). **Risk register impact:** new mitigated risk row — "AI reviewer + drafter share blind spot (both hallucinate same Baymard URL)" (Medium likelihood; mitigated by Tier 2 spot-check on 15-of-30 sample preserving strict R15.3.2 compliance + `neural-heuristic-reviewer` WebFetch URL verification). Net break risk remains zero per §3 (Phase 0b still CONSUMES HeuristicSchemaExtended; the schema amendment IS by Phase 6 owner — see types.ts; Phase 6 loader pre-existing `safeParse` accepts the additive optional field).
  impacted: []
  unchanged: []

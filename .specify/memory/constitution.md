...
> **SYNC COPY — DO NOT EDIT HERE.** This file is a sync'd copy of `docs/specs/mvp/constitution.md` (the canonical Neural project constitution, R1-R26). Edit the canonical file and re-copy via `cp docs/specs/mvp/constitution.md .specify/memory/constitution.md`. The `/speckit.analyze` skill loads this file by default; the copy ensures the analyze constitution-check pass validates against the real rules (without it, the standard Spec Kit boilerplate template would silently no-op the check). Last copied 2026-05-01 (v1.3 sync — Round 3 metadata fix).
